 ...
 */
@CssImport(value = "./styles/views/main/main-view.css",themeFor = "vaadin-dialog-overlay")
public class DialogHeaderBar extends HorizontalLayout {
    ...
    private final HeaderBarButton minimizeButton = new HeaderBarButton(VaadinIcon.MINUS);
    ...
    /**
     * Makes the minimize button visible or invisible.
     *
     * @param isMinimizeVisible
     * @return this
     */
    @NotNull
    public DialogHeaderBar setMinimizeVisible(boolean isMinimizeVisible) {
        getMinimizeButton().setVisible(isMinimizeVisible);
        return this;
    }

    /**
     * @return true if the minimize/restore button is visible (the default).
     */
    public boolean isMinimizeVisible() {
        return getMinimizeButton().isVisible();
    }

    /**
     * Returns true if the dialog is currently minimized.
     * @return true if the dialog is currently minimized.
     */
    ...
    /**
     * Sets the dialog to either "minimized" state (taking all the screen's real estate)
     * or "restored" state (the default).
     * @param minimized true if the dialog should be minimized, false if the dialog should be in the "restored" state.
     * @return this
     */
    @NotNull
    ...
    public DialogHeaderBar(@NotNull Dialog dialog) {
        ...
        getStyle().set("border-bottom", "solid 1px var(--lumo-contrast-10pct)").set("min-height","3rem").set(
                "align-items","center").set(
                "color","white").set(
                "background","#c93434").set(
                "position","absolute");
    ...
    private void update() {
        ...
        dialog.setDraggable(!maximized && !minimized);
        dialog.setResizable(!maximized && !minimized);
        caption.getStyle().set("cursor", maximized && minimized? "default" : "move");
        center(dialog);
    }
    ...
    @NotNull
    HeaderBarButton getMinimizeButton() {
