...
public class MakeDiseasesList extends ListBox<Disease> {
    ...
    public MakeDiseasesList(User user , Mutation mutation, @Autowired UserService userService, @Autowired GeneService geneService ,
                            @Autowired DiseaseService diseaseService , @Autowired MutationService mutationService){
        setWidth("100%");
        setRenderer(new ComponentRenderer<>(disease -> {
            ...
            Span diseaseName = new Span(disease.getDiseaseName());
            Span category = new Span(disease.getClassification().replaceAll("\\|\\$"," , "));
            category.getStyle()
                    .set("color", "var(--lumo-secondary-text-color)")
                    .set("font-size", "var(--lumo-font-size-s)");

            VerticalLayout personalDetails = new VerticalLayout(diseaseName, category);
            Hr line = new Hr();
            line.setWidth("80%");
...
//            });
    ...
    public MakeDiseasesList(User user , Gene gene, @Autowired UserService userService, @Autowired GeneService geneService ,
                            @Autowired DiseaseService diseaseService , @Autowired MutationService mutationService){
        setWidth("100%");
        setRenderer(new ComponentRenderer<>(disease -> {
            HorizontalLayout row = new HorizontalLayout();
