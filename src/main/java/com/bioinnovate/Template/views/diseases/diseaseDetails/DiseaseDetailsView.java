package com.bioinnovate.PreMediT.views.diseases.diseaseDetails;
...
public class DiseaseDetailsView extends Div implements HasUrlParameter<String> {
    ...
    public void setParameter(BeforeEvent event, String parameter) {
        try{
            ...
            if (isLoggedIn()){
                Notification.show("Wrong Disease Id, try navigating using our interface").addThemeVariants(NotificationVariant.LUMO_ERROR);
            }
        }
    }

    @Autowired
    public DiseaseDetailsView(@Autowired UserService userService, @Autowired DiseaseService diseaseService, @Autowired GeneService geneService, @Autowired MutationService mutationService) {
...
//        mutationsLink. setClassName("disease-view-internal-link");
        ...
        HorizontalLayout links = new HorizontalLayout(aliasesAndClassificationsLink,summaryLink,symptomsLink,genesLink,mutationsLink,referencesLink,externalLinksLink);
        links.setWidthFull();
